...
# NOTE: the first process_page_label call above slims _ANALYSIS_CACHE[_OCR_KEY]
# to {"_slimmed": True} (memory optimization in page_label). So we must
# RE-ESTABLISH the full cache entry (with empty words) rather than mutate the
# now-slimmed one, then invalidate the derived cache which memoizes min_conf.
_pl_mod._ANALYSIS_CACHE[_OCR_KEY] = {
    "paragraphs": [
        {"content": "OCR'd content on page 5.",
         "boundingRegions": [{"pageNumber": 5,
